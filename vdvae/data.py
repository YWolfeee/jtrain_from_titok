...
def set_up_imagenet64_preprocess_func(H):
    shift_loss = -127.5
    scale_loss = 1. / 127.5
    if H.dataset == 'imagenet64':
        H.image_size = 64
        H.image_channels = 3
        shift = -115.92961967
        scale = 1. / 69.37404
    ...
    def preprocess_func(x):
        # Different from the native script set_up_data, here x is an image tensor rather than a tuple
        nonlocal shift
        nonlocal scale
        nonlocal shift_loss
        nonlocal scale_loss
...
def cifar10(data_root, one_hot=True):
    raise ValueError('cifar10 is not supported')

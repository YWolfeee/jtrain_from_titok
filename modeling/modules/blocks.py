...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        attn_output = self.attention(x=self.ln_1(x), key_padding_mask=key_padding_mask, attn_mask=attn_mask)
        x = x + attn_output
        if self.mlp_ratio > 0:
...
class TiTokEncoder(nn.Module):
    ...
    def forward(self, pixel_values, latent_tokens, key_padding_mask=None, attn_mask=None):
        ...
        for i in range(self.num_layers):
            x = self.transformer[i](x, key_padding_mask=key_padding_mask, attn_mask=attn_mask)

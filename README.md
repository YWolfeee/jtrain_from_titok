...
2. Data Preparation: in `bash exp_scripts/convert_imagenet.sh`, use your own data dir in `output_dir`, raplace `dataset.params.train_shards_path_or_url` correspondly.
3. Training: `exp_scripts/train.sh`: replace the config path in `config` by `configs/training/stage1/titok_b64_matryoshka.yaml`.

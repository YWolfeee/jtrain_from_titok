...
for config_file in "${config_files[@]}"; do
    ...
    for setting in "${settings[@]}"; do
        ...
        jobname="${config_name}+continue_train+lr=4e-4+use_ours=${use_reconstruction_regularization}+use_annealing=${use_annealing}+is_increasing=${is_increasing}+use_self_distilliation=${use_self_distilliation}"

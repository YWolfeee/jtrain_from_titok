    ...
    for use_ours in "True"; do
        ...
        # rate_weight=1.0
                    ...
                    command="sbatch --job-name=$jobname --output='$output_root/$jobname/logs/slurm_%j.out' \
                        exp_scripts/long_slurm.sh $config_name $per_gpu_batch_size $learning_rate $use_reconstruction_regularization $use_ours $rate_weight $elbo_lower $elbo_upper $output_root"
                    echo "$command"
                    eval "$command"
                # exit
                # done

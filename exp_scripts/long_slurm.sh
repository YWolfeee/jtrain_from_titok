...
echo "Running config: $config_name; batch_size: ${per_gpu_batch_size}; learning_rate: ${learning_rate}; user_reconstruction_regularization: ${use_reconstruction_regularization}; use_gaussian_smoothing: ${use_gaussian_smoothing}; rate_weight: ${rate_weight}; policy_network: ${policy_network}; use_T: ${use_T}; output_root: ${output_root}."
...
function launch_more_jobs() {
    ...
    command="sbatch --job-name=${SLURM_JOB_NAME} --output='${output_root}/${SLURM_JOB_NAME}/logs/slurm_%j.out' exp_scripts/long_slurm.sh $config_name $per_gpu_batch_size $learning_rate $use_reconstruction_regularization $use_gaussian_smoothing $rate_weight $policy_network $use_T $output_root"
...
function do_actual_work() {
    ...
    enroot start --rw --mount /lustre/fsw/portfolios/dir/users/haotiany/joint_training/:/joint_training my_workspace \
        /bin/bash /joint_training/jtrain_from_titok/exp_scripts/main_newloss.sh $config_name $per_gpu_batch_size $learning_rate $use_reconstruction_regularization $use_gaussian_smoothing $rate_weight $policy_network ${use_T} ${output_root} ${SLURM_JOB_NAME}
